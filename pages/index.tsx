...
export default function Home() {
  ...
  const {data, error} = useQuery<HousesQuery, HousesQueryVariables>(
    ...
    if(error) {
      return <Layout main={<div>Error loading houses Rich!</div>} />
    }
    ...
    <Layout
      main={
        <div className="flex">
          ...
          >
            <HouseList houses={lastData ? lastData.houses : []} setHightlightedId={setHightlightedId}/>
          </div>
          <div className="w-1/2">
            <Map setDataBounds={setDataBounds} houses={lastData ? lastData.houses : []} highlightedId={highlightedId}/>
          </div>
        </div>
      }
    />
