...
export default function HouseForm({house}: IProps) {
    ...
    const [createHouse] = useMutation<CreateHouseMutation, CreateHouseMutationVariables>(
        CREATE_HOUSE_MUTATION
    )
    const [updateHouse] = useMutation<UpdateHouseMutation, UpdateHouseMutationVariables> (
    ...
    const handleCreate = async (data: IFormData) => {
        ...
        if (signatureData) {
            ...
            if (houseData?.createHouse) {
                ...
                //error message of your choice here
            }
    ...
    const onSubmit = (data: IFormData) => {
        ...
        //if an edit (if there's a house)
        if(!!house) {
            handleUpdate(house, data)
        } else {
            //if a new record, create new house
           handleCreate(data) 
        }
    }
    ...
    return (
        <form className="mx-auto max-w-xl py-4" onSubmit={handleSubmit(onSubmit)}>
            <h1 className="text-xl">{house ? `Editing ${house.address}` : "Add a New House"}</h1>
            ...
            <div className="mt-4">
                ...
                <input 
                    ...
                    accept="image/*" 
                    style={{display: "none"}} 
                    ref={register({validate: (fileList: FileList) => {
                    ...
                    {previewImage ? <img src={previewImage} className="mt-4 object-cover" style={{width: "576px", height: `${(9/16) * 576}px`}}/> : house ? 
                    <Image 
                        className="mt-4" 
                    ...
                    <div className="mt-4">
                        ...
                        <input 
                            ...
                            ref={register({
                                ...
                                max: {value: 10, message: "Too big! Too many bedrooms! Max is 10"},
                                min: {value: 1, message: "Not enough! Not enough bedrooms! Min is 1"}
                            })}
                        />
